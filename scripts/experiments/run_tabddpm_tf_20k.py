"""Train TabDDPM with Transformer denoiser on shared 20k / seed_42 subset."""
...
def _run_one(
    ...
    payload["tabddpm_t"] = T
    payload["backbone"] = "transformer" if model == "tabddpm_tf" else "mlp"
...
def main() -> None:
    ...
    parser.add_argument("--output_root", type=str, default="exp/tabddpm_tf_20k")
    ...
    parser.add_argument("--tabddpm_t", type=int, default=100)
    parser.add_argument(
        "--models",
        nargs="+",
        default=["tabddpm_tf"],
        choices=["tabddpm", "tabddpm_tf"],
    )
    ...
    summary.to_csv(root / "summary_tabddpm_tf.csv", index=False)
