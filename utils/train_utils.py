...
def sample_trip(model, df, num_samples=0, device=None, match_test_one_to_one=True):
    ...
    Args:
        ...
        df: DataFrame providing conditions (and truth trips for evaluation)
        num_samples: Used only when match_test_one_to_one=False. Number of random
            condition draws with replacement.
        device: Torch device
    ...
    if match_test_one_to_one:
        sampled_rows = df.reset_index(drop=True)
        num_samples = len(sampled_rows)
        print(f"Generating {num_samples} trips with 1:1 test conditions (full test set)...")
    else:
        if num_samples is None or num_samples <= 0:
            raise ValueError("num_samples must be > 0 when match_test_one_to_one=False")
        print(f"Sampling {num_samples} conditions from dataset (with replacement)...")
